...
class GaussianSampler:
    def __init__(self, param_dim=19, population_size=8, sigma=0.05):
        """
        初始化高斯采样器（LLM/GA/PSO/CMA-ES 各方法共用的种群采样与世代记录）
        param_dim: 参数维度
    ...
    def sample_population(self):
        """基于当前均值进行高斯采样生成种群"""
        # 防御：均值维度异常（如续跑文件里残留 8 维）时补齐到 9 维
        if (self.current_mean is None
                or len(self.current_mean.get('A_u', [])) != 9
                or len(self.current_mean.get('A_l', [])) != 9):
            print("[WARN] 采样均值维度异常，重置为初始均值")
            self.initialize()
        population = []
    ...
    def update_mean(self, new_mean):
        """更新世代采样均值（对 LLM 及 GA/PSO/CMA-ES 各优化器通用）"""
        # 验证并修正传入的均值
        self.current_mean = {
            'A_u': np.clip(new_mean['A_u'], self.bounds['A_u'][0], self.bounds['A_u'][1]).tolist(),
            'A_l': np.clip(new_mean['A_l'], self.bounds['A_l'][0], self.bounds['A_l'][1]).tolist(),
        }
