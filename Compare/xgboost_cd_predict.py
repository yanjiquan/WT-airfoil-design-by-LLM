...
CSV_OUTPUT = os.path.join(OUTPUT_DIR, f"{TEST_AIRFOIL}_XGBoost_CD_predictions.csv")
MODEL_OUTPUT = os.path.join(OUTPUT_DIR, f"{TEST_AIRFOIL}_XGBoost_CD_model.json")
REPORT_OUTPUT = os.path.join(OUTPUT_DIR, f"{TEST_AIRFOIL}_XGBoost_CD_evaluation_report.txt")
...
def load_mat_data(mat_path: str):
    ...
    返回 (alpha, cd) 两个 1D numpy 数组，或 (None, None) 表示失败。
    """
    data_matrix = None
...
def mean_relative_error(y_true, y_pred):
    ...
    注意：当真实值 y_true 极接近 0 甚至为负（动态测量噪声）时，MRE/MAPE 会数值爆炸。
    这里用阻力系数物理上有意义的最小量级 CD_MRE_FLOOR = 1e-3 作为分母下限，
    从而避免少量近零样本（例如 CD≈0 的过渡点）主导整体均值。
    """
    CD_MRE_FLOOR = 1e-3  # 对阻力系数而言，0.001 是合理的相对误差参考底限
...
def generate_report(train_df, test_df, model, feature_cols, target_col,
    ...
    log(sep)
    log("  XGBoost 动态俯仰翼型阻力系数 CD 预测 —— 模型评估报告")
    log(sep)
    import datetime
    ...
    train_df["pred_CD"] = y_pred_train
    test_df["pred_CD"] = y_pred_test
    ...
    if tr_up_mask.any():
        m_up = evaluate(
            train_df.loc[tr_up_mask, "CD"].values,
            train_df.loc[tr_up_mask, "pred_CD"].values,
            "训练上升",
        )
        log(_fmt_row("训练上升", m_up, tr_all["n"]))
    if tr_dn_mask.any():
        m_dn = evaluate(
            train_df.loc[tr_dn_mask, "CD"].values,
            train_df.loc[tr_dn_mask, "pred_CD"].values,
            "训练下降",
        )
    ...
    if te_up_mask.any():
        te_up_m = evaluate(
            test_df.loc[te_up_mask, "CD"].values,
            test_df.loc[te_up_mask, "pred_CD"].values,
            "测试上升",
        )
        log(_fmt_row("测试上升", te_up_m, te_all["n"]))
    if te_dn_mask.any():
        te_dn_m = evaluate(
            test_df.loc[te_dn_mask, "CD"].values,
            test_df.loc[te_dn_mask, "pred_CD"].values,
            "测试下降",
        )
...
def main():
    ...
    # ---- 3. 训练 XGBoost ----
    print("\n" + "=" * 70)
    print("步骤 3：训练 XGBoost 回归模型（目标：CD 阻力系数）")
    print("=" * 70)
    ...
    # 输出列顺序：前三列严格 alpha, true_CD, pred_CD
    output_cols = [
        "alpha", "CD", "pred_CD",
    ...
    out_df = test_df_pred[output_cols].rename(columns={"CD": "true_CD"})
