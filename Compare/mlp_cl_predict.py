# -*- coding: utf-8 -*-
"""
PyTorch MLP 回归模型预测动态俯仰翼型升力系数 CL
===================================================
严格实验约束：
...
CSV_OUTPUT = os.path.join(OUTPUT_DIR, f"{TEST_AIRFOIL}_MLP_CL_predictions.csv")
MODEL_OUTPUT = os.path.join(OUTPUT_DIR, f"{TEST_AIRFOIL}_MLP_CL_model.pt")
ARTIFACTS_OUTPUT = os.path.join(OUTPUT_DIR, f"{TEST_AIRFOIL}_MLP_CL_artifacts.pkl")  # X_scaler/y_scaler/feature_names
REPORT_OUTPUT = os.path.join(OUTPUT_DIR, f"{TEST_AIRFOIL}_MLP_CL_evaluation_report.txt")
...
def mean_relative_error(y_true, y_pred):
    """MRE：对 CL 采用 1e-3 作为分母下限，避免近零点爆炸。"""
    CL_MRE_FLOOR = 1e-3
...
def predict_raw(model, X, batch_size=4096):
    """在任意 numpy X 上执行模型前向，返回 (n_samples,) 原始标准化尺度下的预测。"""
    model.eval()
...
def generate_report(artifacts, split_info, meta,
    ...
    log(sep)
    log("  PyTorch MLP 动态俯仰翼型升力系数 CL 预测 —— 模型评估报告")
    log(sep)
...
def main():
    ...
    print("\n" + "=" * 70)
    print("步骤 3：StandardScaler（仅训练集 fit）")
    print("=" * 70)
    ...
    # ---- 4. DataLoader + MLP 模型 ----
    print("\n" + "=" * 70)
    print("步骤 4：构建 DataLoader & MLP 模型")
    print("=" * 70)
    ...
    test_df_out["pred_CL"] = y_test_pred
    ...
    if up_mask.any():
        ...
        te_up = compute_metrics(
            test_df_out.loc[up_mask, "CL"].values,
            test_df_out.loc[up_mask, "pred_CL"].values,
            "测试上升",
        )
    if dn_mask.any():
        print("\n[测试集下降段]")
        te_dn = compute_metrics(
            test_df_out.loc[dn_mask, "CL"].values,
            test_df_out.loc[dn_mask, "pred_CL"].values,
            "测试下降",
        )
    ...
    out_df = test_df_out[output_cols].rename(columns={"CL": "true_CL"})
